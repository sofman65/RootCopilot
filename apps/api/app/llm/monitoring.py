...
class JSONFormatter(logging.Formatter):
    ...
    def format(self, record):
        # Call super so exc_info gets rendered into record.exc_text
        super().format(record)
        log_obj = {
...
class MetricsCollector:
    ...
    def __init__(self):
        # Not thread-safe — swap for atomic Prometheus counters in production
    ...
    def summary(self) -> dict:
        ...
        avg_latency = (
            self._latency_sum / self._requests_total if self._requests_total > 0 else 0.0
        )
        error_rate = (
