...
export async function insertChunks(ctx, { entryId, startOrder, chunks }) {
    ...
    return { status: previousEntry ? "pending" : "ready" };
}
...
export const replaceChunksPage = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (!isLatest) {
            return { status: "replaced", nextStartOrder: startOrder };
        }
        // Get the namespace for filter conversion
        const namespace = await ctx.db.get(entry.namespaceId);
        assert(namespace, `Namespace ${entry.namespaceId} not found`);
        ...
        const pendingEntries = entry.key && previousEntry
            ? (await ctx.db
                ...
                .eq("key", entry.key))
                .collect()).filter((e) => e._id !== entry._id)
            : [];
        ...
        for await (const chunk of chunkStream) {
            ...
            if (chunk.order > indexToDelete) {
                ...
                if (dataUsedSoFar > BANDWIDTH_PER_TRANSACTION_SOFT_LIMIT) {
                    return { status: "pending", nextStartOrder: indexToDelete };
                }
            }
        ...
        await handleBatch();
        return { status: "ready", nextStartOrder: 0 };
    },
});
...
export async function deleteChunksPageHandler(ctx, { entryId, startOrder }) {
...
function estimateEmbeddingSize(embedding) {
    let dataUsedSoFar = embedding.vector.length * 8 +
