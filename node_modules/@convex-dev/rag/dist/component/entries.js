...
export const list = query({
    ...
    handler: async (ctx, args) => {
        ...
        const results = await stream(ctx.db, schema)
            .query("entries")
            .withIndex("status_namespaceId", (q) => namespaceId
...
async function getEntriesByKey(ctx, args) {
    return mergedStream(statuses.map((status) => stream(ctx.db, schema)
        .query("entries")
        .withIndex("namespaceId_status_key_version", (q) => q
...
export const deleteByKeySync = action({
    ...
    handler: async (ctx, args) => {
        while (true) {
            const entries = await ctx.runQuery(internal.entries.getEntriesForNamespaceByKey, { namespaceId: args.namespaceId, key: args.key });
            for await (const entry of entries) {
